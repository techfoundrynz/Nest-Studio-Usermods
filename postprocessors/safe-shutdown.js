 ...
 */
const code = (line) => line.replace(/\([^)]*\)/g, "").replace(/;.*$/, "").toUpperCase();
...
module.exports = {
  ...
  stages: ["export"],
  process(gcode, ctx) {
    const s = { spindleOff: true, coolantOff: true, programEnd: true, ...ctx.settings };
    const lines = gcode.split(/\r?\n/);
    let endIndex = -1;
    let spindleOn = false, coolantOn = false;
