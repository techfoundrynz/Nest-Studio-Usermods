...
(function modsMenu() {
  ...
  rt.register({ name: "mods-menu", version: "0.3.0" });
  ...
  function onOutside(event) {
    if (panel && !panel.contains(event.target) && !event.target.closest(`#${BUTTON_ID}`)) closePanel();
  }

  const btn = (label, onClick, title) => rt.el("button", { class: "usermod-btn", text: label, title, onClick });
  ...
  async function render(button) {
    ...
    const withDesc = (name, desc) => [name, desc ? rt.el("small", { text: ` — ${desc}` }) : null];
            ...
            btn(
              ...
              async () => {
                try {
                  ...
                  rt.toast(`${a.label}: ${error.message}`, { kind: "error" });
                }
              },
              a.title
            )
    ...
    panel.replaceChildren(
      ...
      d.errors.length ? rt.el("h4", { text: "Recent errors", class: "err" }) : null,
      d.errors.length ? list(d.errors.slice(-5), (e) => [`${e.scope}: ${e.message}`]) : null,
      rt.el("h4", { text: "Maintenance" }),
      rt.el("div", { class: "row" }, [
        btn("Open mod folder", () => window.usermod.openModDir()),
        btn(
          ...
          "Re-reads postprocessors/*.js and mods.json without restarting"
        ),
        btn(
          "Reload UI",
          () => {
  ...
  async function togglePanel(button) {
    if (!button) return;
    if (panel) {
      closePanel();
      return;
    ...
    try {
      ...
      panel.textContent = `usermod error: ${error.message}`;
    }
  }
  ...
  window.usermodMenu = { toggle: () => togglePanel(document.getElementById(BUTTON_ID)), close: closePanel };

  rt.waitFor(SETTINGS_SELECTOR, { timeout: 60000 }).then(mount).catch((e) => rt.log("warn", e.message));
  rt.observe(() => {
