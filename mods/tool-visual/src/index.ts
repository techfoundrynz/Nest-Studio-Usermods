...
(function toolVisual(): void {
  ...
  rt.register({ name: "tool-visual", version: "0.2.0" });
  ...
  function applyForSample(runtime: UsermodSimRuntimeLike, state: Hooked, sampleIndex: number): void {
    ...
    if (!spec) {
      ...
      if (gcodeId && !warned.has(gcodeId)) {
        warned.add(gcodeId);
        rt.log("warn", `tool-visual: no tool data for toolpath ${gcodeId} (scene map ${state.tools.size}, list rows ${rows.size}, library ${library.size}); default cutter kept`);
      }
      return;
    }
