/*
 * Feeds & speeds: a calculator that starts from the tool library (diameter, flute count) and a material preset
 * (chip load per tooth, surface-speed range, depth and stepover rules) and gives feed, plunge feed, depth of
 * cut and stepover for a chosen spindle speed. Advisory starting points for a light desktop machine; the
 * operation's own feed and speed still live in the CAM setup, because the tool library stores geometry only.
 *
 *   feed = chip load x flutes x rpm          Vc = pi x diameter x rpm / 1000 (m/min)
 *
...
(function feedsSpeeds(): void {
  ...
  rt.register({ name: "feeds-speeds", version: "0.2.0" });
