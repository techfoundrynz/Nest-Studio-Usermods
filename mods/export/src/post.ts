/*
 * Export (post-processor, runs last): analyses the final G-code of every export and writes a JSON report
 ...
 *
 * mods.json settings ("export", shared with the main and UI halves): keep (50), stockThickness (mm or null),
 *   useHeaderStock (true), allowanceBelow (0.5), safeZ (2), checkRapids (true), xMin/xMax/yMin/yMax (null = off),
 *   copyTo ("" = no extra copy), copySubfolderByDate (false), copyOverwrite (true), template ("{name}"), saveDir ("")
 */
...
/** The optional extra copy of the finished program (a USB stick, a network share the machine PC reads). */
function copyOut(gcode: string, ctx: Usermod.PostprocessorContext<Settings>, s: Settings): void {
  if (!s.copyTo || !ctx.fileName) return;
  let dir = s.copyTo;
  if (s.copySubfolderByDate) dir = path.join(dir, new Date().toISOString().slice(0, 10));
  try {
    fs.mkdirSync(dir, { recursive: true });
    const target = path.join(dir, path.basename(ctx.fileName));
    if (!s.copyOverwrite && fs.existsSync(target)) {
...
const mod: Usermod.Postprocessor<Settings> = {
  ...
  stages: ["export"],
  process(gcode, ctx) {
    const s: Settings = { keep: 50, copyTo: "", copySubfolderByDate: false, copyOverwrite: true, stockThickness: null, useHeaderStock: true, allowanceBelow: 0.5, safeZ: 2, checkRapids: true, xMin: null, xMax: null, yMin: null, yMax: null, ...ctx.settings };
